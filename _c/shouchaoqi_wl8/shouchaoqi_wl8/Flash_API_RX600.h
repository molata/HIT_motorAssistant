...
#define RX62T     1
    ...
    #error "ERROR !!! You can only have one RX6xx Device defined at a time in Flash_API_RX600.h !!!"
#endif
 ...
 *________________________________________________________________________*/
 /* Max System Clock for RX600 is 100MHz, but 48MHz is needed for USB on
  ...
  #error "ERROR !!! You must specify the System Clock Frequency (ICLK_FREQUENCY) in Flash_API_RX6xx.h first !!!";
#endif
...
/* Max Peripheral Clock for RX600 is 50MHz, but USB needs 48MHz */
  ...
  #error "ERROR !!! You must specify the Peripheral Clock Frequency (PCLK_FREQUENCY) in Flash_API_RX6xx.h first !!!";
#endif
...
/* a different memory layout                                               */
    ...
    /*  According to HW Manual the Max Programming Time for 256 bytes (ROM) is 12ms.  This is with a PCLK of 50MHz.
        The calculation below calculates the number of ICLK ticks needed for the timeout delay.  The 12ms number
        is adjusted linearly depending on the PCLK frequency. */
    #define		WAIT_MAX_ROM_WRITE		((int32_t)(12000 * (50.0/PCLK_FREQUENCY))*ICLK_FREQUENCY)		       
    /*  According to HW Manual the Max Programming Time for 128 bytes (Data Flash) is 5ms.  This is with a PCLK of 50MHz.
        The calculation below calculates the number of ICLK ticks needed for the timeout delay.  The 12ms number
        is adjusted linearly depending on the PCLK frequency. */
    #define		WAIT_MAX_DF_WRITE		((int32_t)(5000 * (50.0/PCLK_FREQUENCY))*ICLK_FREQUENCY)
#else
    #error "ERROR !!! Define timeout values for this device in Flash_API_RX600.h !!!"
#endif
...
#else
    #error "ERROR !!! Set memory locations for FCU RAM and ROM in Flash_API_RX600.h !!!"
#endif
...
/* Memory specifics for the RX610 group */
    ...
    #define DEFINE_ADDR_LOOKUP const uint32_t block_addresses[32] = { \
            0x00FFE000,  /* EB00 Caution. ID CODE(FFFFFFA0-FFFFFFAF) is excluded. */ \
            0x00FFC000,  /* EB01 */ \
            0x00FFA000,  /* EB02 */ \
            0x00FF8000,  /* EB03 */ \
            0x00FF6000,  /* EB04 */ \
    ...
    /*  According to HW Manual the Max Erasure Time for a 128kB block is 1750ms.  This is with a PCLK of 50MHz.
        The calculation below calculates the number of ICLK ticks needed for the timeout delay.  The 1750ms number
        is adjusted linearly depending on the PCLK frequency. */
    #define 	WAIT_MAX_ERASE	    	((int32_t)(1750000 * (50.0/PCLK_FREQUENCY))*ICLK_FREQUENCY) 	
...
/* Memory specifics for the RX621/N group */
    ...
    #define DEFINE_ADDR_LOOKUP const uint32_t block_addresses[54] = { \
            0x00FFF000,  /* EB00 Caution. ID CODE(FFFFFFA0-FFFFFFAF) is excluded. */ \
            0x00FFE000,  /* EB01 */ \
            0x00FFD000,  /* EB02 */ \
            0x00FFC000,  /* EB03 */ \
            0x00FFB000,  /* EB04 */ \
    ...
    /*  According to HW Manual the Max Erasure Time for a 16kB block is around 300ms.  This is with a PCLK of 50MHz.
        The calculation below calculates the number of ICLK ticks needed for the timeout delay.  The 300ms number
        is adjusted linearly depending on the PCLK frequency. */
    #define 	WAIT_MAX_ERASE	    	((int32_t)(300000 * (50.0/PCLK_FREQUENCY))*ICLK_FREQUENCY) 	
...
/* Memory specifics for the RX62T group */
    ...
    /* The Data Flash size on the RX62N RSK is 8KB */
    #define DF_SIZE             8192
...
#else
    #error "!!! Need to define memory specifics for this RX600 family in Flash_API_RX600.h !!!"
#endif
